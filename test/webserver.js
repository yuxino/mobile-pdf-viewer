...
var http = require(process.env.HTTPS ? "https" : "http");
...
options.key = options.cert = createCertificate();

function WebServer() {
  this.root = ".";
  this.host = "0.0.0.0";
  this.port = process.env.HTTPS ? 443 : 80;
  this.server = null;
  this.verbose = false;
...
WebServer.prototype = {
  start: function (callback) {
    ...
    this.server = http.createServer(options, this._handler.bind(this));
  ...
  _ensureNonZeroPort: function () {
    if (!this.port) {
      ...
      var server = http.createServer(options).listen(0);
        ...
        files.forEach(function (file) {
          ...
          if (stat) {
            if (stat.isDirectory()) {
              ...
              href = "/web/viewer.html?url=" + encodeURIComponent(item);
              label = file;
